...
if BUCKET_NAME is None:
    raise ValueError(
        "A variável de ambiente 'S3_BUCKET_NAME' não está definida."
    )
...
def Analisando_Dados(df: pd.DataFrame) -> pd.DataFrame:
    df = df.sort_values(by=["reference_date"])

    variacao_percentual = (
        df["close_price"].pct_change().mul(100).round(2)
    )
    df["variacao_percentual"] = variacao_percentual
...
def executar_ouro(silver_key: str, symbol: str) -> str:
